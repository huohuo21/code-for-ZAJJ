...
void draw_eff(std::string name, out_data eff_data, out_data eff_mc)
{
	TCanvas* c1 = new TCanvas("c","c",800,600);
        ...
        TH2D *h2D = new TH2D("2Dh","HLT_Mu17_TacIsoVVL behavior",4,abseta1,4,abseta2);
	...
	for (int i=0; i<4; i++)
        {
                for (int j=0; j<4; j++)
                {
                        h2D->Fill(abseta1[i]+0.1, abseta2[j]+0.1, SFs[3-i][j]);
                        h2D->SetBinError(i+1,j+1,SFs_err[3-i][j]);
                }
        }
	h2D->SetMinimum(0.7);
        h2D->SetMaximum(1.0);
