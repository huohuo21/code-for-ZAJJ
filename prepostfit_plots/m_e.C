...
TH1F* construct_th(std::string str, TDirectoryFile* fout){
	TDirectoryFile* d1 = (TDirectoryFile*)fout->Get("ch28");
	TDirectoryFile* d2 = (TDirectoryFile*)fout->Get("ch29");
	TDirectoryFile* d3 = (TDirectoryFile*)fout->Get("ch30");
	TDirectoryFile* d4 = (TDirectoryFile*)fout->Get("ch31");
	TDirectoryFile* d5 = (TDirectoryFile*)fout->Get("ch32");
	TDirectoryFile* d6 = (TDirectoryFile*)fout->Get("ch33");
	TDirectoryFile* d7 = (TDirectoryFile*)fout->Get("ch34");
	TDirectoryFile* d8 = (TDirectoryFile*)fout->Get("ch35");
...
//	TDirectoryFile* ds = (TDirectoryFile*)fout->Get("shapes_prefit");
	...
	t_data->SetBinContent(1,25);
	t_data->SetBinContent(2,3);
	t_data->SetBinContent(3,2);
	t_data->SetBinContent(4,12);
	t_data->SetBinContent(5,7);
	t_data->SetBinContent(6,1);
	t_data->SetBinContent(7,6);
	t_data->SetBinContent(8,6);

	THStack* hs = new THStack("hs", "");
        hs->SetMaximum(t_ZA->GetBinContent(1)*4.2);
        hs->Add(t_ST);
        hs->Add(t_TTA);
        hs->Add(t_VV);
        hs->Add(t_WA);
	...
	leg2->AddEntry(t_VBS, "EWK Z#gamma");
        ...
        TArrow* line1 = new TArrow(t_ZA->GetBinLowEdge(4),0,t_ZA->GetBinLowEdge(4),3.5*t_ZA->GetMaximum(),0.07,"") ;
        TArrow* line2 = new TArrow(t_ZA->GetBinLowEdge(7),0,t_ZA->GetBinLowEdge(7),3.*t_ZA->GetMaximum(),0.07,"") ;
