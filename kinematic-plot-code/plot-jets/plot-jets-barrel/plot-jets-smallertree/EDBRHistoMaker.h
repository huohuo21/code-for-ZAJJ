...
void EDBRHistoMaker::createAllHistos() {
	...
	hs.setHisto("jet1eta", 15, -2.5, 2.5);
	hs.setHisto("jet2eta", 15, -2.5, 2.5);
...
void EDBRHistoMaker::Loop(std::string outFileName) {
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		if (((run_period<7 && HLT_Mu5 > 0)|| (run_period==7 && HLT_Mu2 > 0)) && lep == 13 && nlooseeles == 0 && nloosemus < 3 && massVlep > 70 && massVlep < 110 && ptlep1 > 20. && ptlep2 > 20. && fabs(etalep1) < 2.4 && fabs(etalep2) < 2.4 && photonet > 25. && fabs(photoneta) < 1.4442 ) {
			numbe_out++;
			treename->Fill();
		}
		else
...
void EDBRHistoMaker::Loop_SFs_mc(std::string outFileName, TH2F* ID_photon, TH2F* ID_BF, TH2F* ID_GH, TH2F* ISO_BF, TH2F* ISO_GH, TGraphAsymmErrors* track_SF, TH2D* di_lep_trigger){
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		if (lep == 13 && HLT_Mu2 > 0 && nlooseeles == 0 && nloosemus < 3 && massVlep > 70 && massVlep < 110 && massVlep < 110 && ptlep1 > 20. && ptlep2 > 20. && fabs(etalep1) < 2.4 && fabs(etalep2) < 2.4 && photonet > 25. && fabs(photoneta) < 1.4442 ) {
			numbe_out++;
			treename->Fill();
		}
		else
		...
		photon_eveto=0.9938;//0.9938;//barrel

		if(l1_weight==0) l1_weight_tmp=0.524;
		if(l1_weight==0.5) l1_weight_tmp=0.762;
		if(l1_weight==1) l1_weight_tmp=1;
		actualWeight = actualWeight*ID_lep1*ID_lep2*ISO_lep1*ISO_lep2*Track_lep1*Track_lep2*trigger_SF*photon_ID_scale*photon_eveto*l1_weight_tmp;
		int iswjets = 0;
		int isnotwets = 0;
		int iszjets = 0;
		int isttjets = 0;
