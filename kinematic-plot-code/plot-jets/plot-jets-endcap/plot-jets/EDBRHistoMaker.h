...
void EDBRHistoMaker::createAllHistos() {
	...
	/*hs.setHisto("nVtx", 20, -0.5, 35);
        ...
        hs.setHisto("photoneta", 40, -2.5, 2.5);
	...
	hs.setHisto("nVtx", 20, -0.5, 35);
        ...
        hs.setHisto("photonsieie", 20, 0, 0.03001);
        hs.setHisto("photonphoiso", 10, 0, 2.5);
        hs.setHisto("ptlep1", 15, 20, 300);
        hs.setHisto("etalep1", 15, -2.5, 2.5);
        hs.setHisto("ptlep2", 15, 20, 300);
...
void EDBRHistoMaker::Loop(std::string outFileName) {
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		if (lep == 13 && photonet > 25. && ((run_period<7 && HLT_Mu5 > 0)|| (run_period==7 && HLT_Mu2 > 0)) && fabs(photoneta) < 2.5 && fabs(photoneta) > 1.566 && ptlep1 > 20. && ptlep2 > 20. && fabs(etalep1) < 2.4 && fabs(etalep2) < 2.4
...
void EDBRHistoMaker::Loop_SFs_mc(std::string outFileName, TH2F* ID_photon, TH2F* ID_BF, TH2F* ID_GH, TH2F* ISO_BF, TH2F* ISO_GH, TGraphAsymmErrors* track_SF, TH2D* di_lep_trigger){
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		if (lep == 13 && photonet > 25. && HLT_Mu2 > 0
				&& fabs(photoneta) < 2.5 && fabs(photoneta) > 1.566 && ptlep1 > 20.
		...
		Double_t ID_lep1, ID_lep2, ISO_lep1, ISO_lep2, Track_lep1, Track_lep2, trigger_SF, photon_ID_scale, photon_eveto, ptlep1_tmp, ptlep2_tmp;
		ptlep1_tmp = ptlep1;
		ptlep2_tmp = ptlep2;
