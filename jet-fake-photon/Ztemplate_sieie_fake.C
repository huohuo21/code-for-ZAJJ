...
#include "TTree.h"
#include "TH1.h"
void Ztemplate_sieie_fake(std::string filename, float lowpt, float highpt) {

	std::string pathToTrees = "./photon-rawsigmaetaeta-chiso4to10/";

	TFile* f1 = TFile::Open((pathToTrees + filename + "_for_fakerate_use_fake.root").c_str());
...
////// begin processing photon candidates (medium photon)/////////
	...
	TFile fdata(Form("%s_%0.f_%0.f_fake.root", filename.c_str(), lowpt, highpt),"RECREATE");
	...
	for (Int_t i = 0; i < entrynum; i++) {
		...
		photonpt<highpt && ptlep2 > ptlep_criti && massVlep > massVlep_low_criti && massVlep < massVlep_high_criti) {
			if (filename=="ZJets" && isprompt!=1)histo->Fill(raw_sigmaetaeta, weight_lack_lumi*lumi);
